...
const formSchema = z.object({
    ...
    licenseNumber: z.string().min(8, "License number must be at least 8 characters long"),
    ...
    professionalBio: z.string().min(20, "You must have a brief but detailed professional bio").max(500, "Your professional bio is too long. You need to summarize it"),
        ...
        <div class="py-12">
            <div class="max-w-3xl mx-auto">
                ...
                <UICard>
                    ...
                    <UICardContent>
                        <form id="staff-registration-form" @submit.prevent.stop="form.handleSubmit()">
                            <div class="my-4">
                                ...
                                <form.Field name="fullName">
                                    <template #default="{ field }">
                                        <UIField>
                                            ...
                                            <UIInput id="fullName" v-model="field.state.value" type="text"
                                                :name="field.name" placeholder="Enter your full name"
                            ...
                            <div class="mt-8 mb-4">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <UILabel class="text-lg font-medium">Professional Details</UILabel>
                                </div>
                            </div>
                            <UIFieldGroup class="grid grid-cols-1 md:grid-cols-2">
                                <form.Field name="licenseNumber">
                                    <template #default="{ field }">
                                        <UIField>
                                            <UIFieldLabel for="licenseNumber">Pharmacist Registration Number
                                            </UIFieldLabel>
                                            <UIInput id="licenseNumber" v-model="field.state.value" :name="field.name"
                                                type="text" placeholder="Enter pharmacist registration number"
                                                @blur="field.handleBlur"
                                                @input="(e: Event) => field.handleChange((e.target as HTMLInputElement).value)" />
                                ...
                                <form.Field name="qualifications">
                                    <template #default="{ field }">
                                        <UIField>
                                            <UIFieldLabel for="qualifications">Qualifications
                                            </UIFieldLabel>
                                            <UIInput id="qualifications" v-model="field.state.value" :name="field.name"
                                                type="text" placeholder="Eg PharmD, BPharma etc"
                                                @blur="field.handleBlur"
                                                @input="(e: Event) => field.handleChange((e.target as HTMLInputElement).value)" />
                                ...
                                <form.Field name="hourlyRate">
                                    <template #default="{ field }">
                                        <UIField>
                                            <UINumberField id="hourlyRate" :model-value="field.state.value" :min="100"
                                                :name="field.name" :format-options="{
                                                    style: 'currency',
                                                    currency: 'KES',
                            ...
                            <UIFieldGroup class="grid grid-cols-1 w-full my-4">
                                <form.Field name="professionalBio">
                                    <template #default="{ field }">
                                        <UIField>
                                            <UIFieldLabel for="professionalBio">Professional Bio</UIFieldLabel>
                                            <UITextarea id="professionalBio" v-model="field.state.value"
                                                :name="field.name"
                            ...
                            <div class="mt-8 mb-4">
                                <div class="flex items-center space-x-2">
                                    <Icon name="lucide:stethoscope" class="h-12 w-12 text-muted-foreground" />
                                    <UILabel class="text-lg font-medium">Skills and Specialization</UILabel>
                                </div>
                            </div>
                                    ...
                                    <template #default="{ field }">
                                        <UIField>
                                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 my-3">
                                                <div v-for="service of skills.pharmacistServices" :key="service.id"
                                                    class="flex items-center space-x-2">
                                                    <UICheckbox :id="`${service.name}-${service.id}`"
                                                        :value="service.name"
                                                        :checked="field.state.value.includes(service.name)"
                                                        @update:model-value="(checked: boolean | 'indeterminate') => {
                                                            if (checked === 'indeterminate') return;
                                                            const updateServices = checked ? [...field.state.value, service.name] : field.state.value.filter(s => s !== service.name);
                            ...
                            <div class="flex justify-center">
                                <UIButton type="submit" class="w-[90%] mt-8 bg-gradient-primary">Register as a
                                    Pharmacist
                                </UIButton>
                            </div>
                        </form>
